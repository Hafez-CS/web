...
// Consultant Actions
      ...
      const EditAdminConsultant = async (id : number , data : AddConsultant) => {
        const res = await http.patch(`api/admin-platform/consultants/${id}/`, data, {
          headers: getHeader(),
        });
        return res.data;
      };
    ...
    return {consultResult , TotalResult , GetAdminConsultantList , CreateAdminConsultant , EditAdminConsultant , DeleteAdminConsultant}
}